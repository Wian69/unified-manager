...
import { Search, CalendarDays, CheckCircle2, RefreshCw, AlertTriangle, ArrowRight } from "lucide-react";
        ...
        <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 mt-10">
            <div className="bg-slate-900/80 backdrop-blur-xl p-10 rounded-3xl border border-blue-500/30 shadow-2xl relative overflow-hidden">
                ...
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h1 className="text-4xl font-black text-white tracking-tight">Calendar Delegation</h1>
                        <p className="text-slate-400 mt-1 font-mono text-sm">Instantly grant someone access to another employee's calendar.</p>
                    </div>
                </div>
                    ...
                    <div className="bg-slate-950/50 rounded-2xl border border-slate-800 p-8 space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                        <div className="flex items-start justify-between">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <h2 className="text-2xl font-black text-white">{selectedUser.displayName}'s Calendar</h2>
                                    <p className="text-slate-400 font-mono">{selectedUser.userPrincipalName}</p>
                                    <p className="text-blue-500 text-xs mt-1 uppercase font-bold tracking-widest">Calendar Owner</p>
                                </div>
                            </div>
                            <button 
                                onClick={() => {
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-slate-800/60">
                            <div className="space-y-3">
                                <label className="text-xs font-bold uppercase tracking-widest text-slate-400">Step 2: Who Needs Access?</label>
                                <input 
                                    type="email" 
                            ...
                            <div className="space-y-3">
                                <label className="text-xs font-bold uppercase tracking-widest text-slate-400">Step 3: Permission Level</label>
                                <select 
                                    ...
                                    disabled={shareComplete || isSharing}
                                >
                                    <option value="read">Read Only (Can view all details)</option>
                                    <option value="write">Editor (Can edit events)</option>
                                    <option value="delegateWithoutPrivateEventAccess">Delegate (Cannot see private events)</option>
                                    <option value="delegateWithPrivateEventAccess">Delegate (Can see private events)</option>
                                </select>
                            </div>
                        </div>
                        ...
                        {shareComplete && (
                            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-6 flex items-start gap-4">
                                ...
                                <div>
                                    <h3 className="text-emerald-500 font-black text-lg">Calendar Shared Successfully!</h3>
                                    <p className="text-emerald-400/80 text-sm mt-1 mb-4">
                                        <strong>{targetEmail}</strong> now has <strong>{role}</strong> access to {selectedUser.displayName}'s calendar. They can open it directly in Outlook.
                                    </p>
                                    <button 
                                        onClick={() => {
                                            setShareComplete(false);
                        ...
                        {!shareComplete && (
                            <div className="pt-4 border-t border-slate-800/60">
                                <button 
                                    ...
                                    className="w-full flex items-center justify-center gap-3 bg-blue-600 hover:bg-blue-500 text-white p-5 rounded-xl font-black text-lg shadow-lg shadow-blue-600/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {isSharing ? (
                                        <>

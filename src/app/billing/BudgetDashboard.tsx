...
export default function BudgetDashboard({ 
    ...
    azureRunRate: number,
    billingRegions: any[],
    configurableUsers: {name: string, email: string, currentRegion: string}[]
}) {
    ...
    const [invoiceFilter, setInvoiceFilter] = useState('12 Months');
    ...
    const fetchInvoiceHistory = async (filter = invoiceFilter, start = customStartDate, end = customEndDate) => {
    ...
    return (
        <div className="space-y-12 mb-16">
            ...
            {(() => {
                ...
                if (configurableUsers.length === 0) return null;
                ...
                return (
                    <div className="mt-12 bg-slate-900/50 border border-orange-500/30 rounded-2xl p-6 print:hidden">
                        <div className="flex items-center gap-3 mb-4 border-b border-slate-800 pb-4">
                            ...
                            <div>
                                <h3 className="text-xl font-bold text-white">Manual Region Assignments</h3>
                                <p className="text-sm text-slate-400">Assign unassigned and partner accounts to a specific region to bill their M365 costs appropriately.</p>
                            </div>
                        </div>
                            ...
                            {configurableUsers.map((u) => (
                                <div key={u.email} className="bg-slate-950 border border-slate-800 rounded-xl p-4 flex flex-col justify-between gap-4 relative">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <span className="text-xs text-slate-500 font-medium ml-1">Currently billed to:</span>
                                        <select 
                                            ...
                                            value={u.currentRegion}
                                        >
                                            <option value="" disabled>Assign to Region...</option>
                                            <option value="Unassigned Region">Unassigned Region</option>
                        ...
                        <div className="p-6 space-y-4">
                            {emailSuccess ? (
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        <label className="text-xs font-medium text-slate-400 mb-1 block">From Email</label>
                                        <input 
                                            type="email" 
                                            value={emailFrom}
                                            disabled
                                        ...
                                        />
                                        <p className="text-[10px] text-slate-500 mt-1">Invoices are always sent from IT Support.</p>
                                    </div>
                                    <div className="flex gap-3 mt-6">
                                        ...
                                        <button 
                                            ...
                                            className="flex-1 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-sm font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                                        >
                                            {isSendingEmail ? (
                                                <>
                    ...
                    <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 md:p-8 w-full max-w-3xl shadow-2xl">
                        <div className="flex justify-between items-center mb-6">
                            <div>
                                ...
                                <p className="text-slate-400 text-sm">Official invoices dynamically fetched from Azure Billing API.</p>
                            </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-4 mb-6">
                            <select 
                                ...
                                onChange={(e) => {
                                    ...
                                    if (e.target.value !== 'Custom Range') {
                                        fetchInvoiceHistory(e.target.value);
                                    }
                                }}
                            ...
                            {invoiceFilter === 'Custom Range' && (
                                <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <button 
                                        onClick={() => fetchInvoiceHistory('Custom Range', customStartDate, customEndDate)}
                                        disabled={!customStartDate || !customEndDate}
                                        className="bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-medium py-2 px-4 rounded-lg transition-colors text-sm"
                                    >
                                        Apply
                                    </button>
                                </div>
                                ...
                                <table className="w-full text-left text-sm text-slate-300">
                                    ...
                                    <tbody className="divide-y divide-slate-800/50">
                                        {invoiceHistory.length === 0 ? (
                                            ...
                                            <tr 
                                                key={inv.id} 
                                                onClick={() => inv.pdfUrl ? window.open(inv.pdfUrl, '_blank') : null}
                                                className={`transition-colors ${inv.pdfUrl ? 'hover:bg-slate-800/40 cursor-pointer group' : 'hover:bg-slate-800/20'}`}
                                            >
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    {inv.pdfUrl ? (
                                                        <span className="text-blue-400 group-hover:text-blue-300 flex justify-end items-center gap-1 font-medium">

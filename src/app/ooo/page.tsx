...
export default function OOOManagementPage() {
    ...
    const handleSaveOOO = async () => {
        ...
        alert(`Sync Complete: ${successCount} users updated.`);
    ...
    const ToolbarButton = ({ onClick, children, title }: any) => (
        <button 
            type="button"
            onMouseDown={(e) => {
                e.preventDefault(); // This PREVENTS focus loss and React crashes
                onClick();
            }}
    ...
    const RichTextToolbar = ({ onCommand }: { onCommand: (cmd: string, val: string) => void }) => (
        <div className="flex items-center gap-4 p-3 bg-slate-900 border-b border-slate-800 rounded-t-2xl">
            <div className="flex items-center gap-2 border-r border-slate-700 pr-4">
                <Type size={14} className="text-blue-400" />
                <select 
                    onMouseDown={(e) => e.stopPropagation()}
                    onChange={(e) => onCommand('fontName', e.target.value)}
                    className="bg-slate-950 border border-slate-700 text-slate-300 text-[10px] font-black uppercase py-1.5 px-3 rounded-lg outline-none"
                >
            ...
            <div className="flex items-center gap-1">
                <ToolbarButton onClick={() => onCommand('bold', '')} title="Bold">B</ToolbarButton>
                <ToolbarButton onClick={() => onCommand('italic', '')} title="Italic">I</ToolbarButton>
                <ToolbarButton onClick={() => onCommand('underline', '')} title="Underline">U</ToolbarButton>
            </div>
        </div>
    );
    ...
    return (
        <div className="flex h-[calc(100vh-8rem)] -m-8 bg-slate-950 overflow-hidden relative">
            <div className="absolute top-2 right-2 text-[8px] font-black text-slate-800 pointer-events-none z-50">VER-1.3.0-ISOLATION-FIX</div>
                ...
                <div className="p-6 border-b border-slate-900 space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        {filteredUsers.length > 0 && (
                            <button onClick={() => setSelectedUserIds(selectedUserIds.length === filteredUsers.length ? [] : filteredUsers.map(u => u.id))} className="text-[10px] font-black uppercase text-slate-500 hover:text-blue-400">
                                {selectedUserIds.length === filteredUsers.length ? 'Deselect' : 'Select All'}
                            </button>
                        )}
                    </div>
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={16} />
                        <input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-xl pl-10 pr-4 py-2 text-sm text-white focus:outline-none focus:border-blue-600" />
                    </div>
                </div>

                <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
                    {loadingUsers ? (<Loader2 className="animate-spin text-blue-600 mx-auto mt-12" />) : filteredUsers.map(user => (
                        <div key={user.id} onClick={() => toggleUserSelection(user.id)} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer ${selectedUserIds.includes(user.id) ? 'bg-blue-600/10 border border-blue-600/30' : 'hover:bg-slate-900 border border-transparent'}`}>
                             ...
                             <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between"><div className={`text-sm font-bold truncate ${selectedUserIds.includes(user.id) ? 'text-white' : 'text-slate-200'}`}>{user.displayName}</div>{oooStatusMap[user.id] && oooStatusMap[user.id] !== 'disabled' && <span className="bg-emerald-500/10 text-emerald-500 text-[8px] font-black uppercase px-1.5 py-0.5 rounded-md">Active</span>}</div>
            ...
            <div className="flex-1 bg-slate-950 flex flex-col min-w-0">
                {selectedUserIds.length === 0 ? (
                    <div className="flex-1 flex flex-col items-center justify-center text-slate-500 space-y-4"><Clock size={48} className="text-slate-700" /><h3 className="text-xl font-bold uppercase tracking-tight">Select Users to Begin</h3></div>
                ) : (
                    <div className="flex-1 flex flex-col p-8 overflow-y-auto">
                        <header className="flex items-start justify-between mb-12">
                            <div className="flex items-center gap-6">
                                <div className={`p-4 rounded-2xl shadow-2xl ${isBulkMode ? 'bg-blue-600' : 'bg-slate-900 border border-slate-800'}`}>{isBulkMode ? <Users size={40} className="text-white" /> : <UserCircle size={40} className="text-blue-500" />}</div>
                                <div><h2 className="text-3xl font-black text-white uppercase tracking-tight">{isBulkMode ? `Bulk Update: ${selectedUserIds.length} Users` : activeUser?.displayName}</h2><p className="text-slate-400 font-medium text-sm">{isBulkMode ? 'Multi-Selection Active' : activeUser?.userPrincipalName}</p></div>
                            </div>
                            <button onClick={handleSaveOOO} disabled={savingMailbox || loadingMailbox} className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-30 text-white font-black uppercase text-xs px-8 py-4 rounded-2xl transition-all shadow-xl shadow-emerald-900/20">{savingMailbox ? <RefreshCw className="animate-spin mr-2 inline" /> : <Save className="mr-2 inline" />} {isBulkMode ? 'Apply Bulk' : 'Sync to Outlook'}</button>
                        </header>

                        {loadingMailbox ? (
                            <div className="flex-1 flex flex-col items-center justify-center p-20"><Loader2 className="animate-spin text-blue-600" size={32} /><p className="text-slate-500 mt-4">Syncing with Exchange...</p></div>
                        ) : mailboxSettings ? (
                            <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                                <div className="space-y-8">
                                    <div className="p-8 bg-slate-900 border border-slate-800 rounded-3xl shadow-xl space-y-6">
                                        <div className="flex items-center gap-3 text-lg font-bold text-white uppercase"><RefreshCw size={20} className="text-blue-500" />Auto-Reply Configuration</div>
                                ...
                                <div className="space-y-8">
                                    <div className="p-8 bg-slate-900 border border-slate-800 rounded-3xl shadow-xl space-y-6">
                                        <div className="flex items-center gap-3 text-lg font-bold text-white uppercase"><Mail size={20} className="text-emerald-500" />Message Management</div>
                                        <div className="space-y-8">
                                            {/* RICH EDITORS ISOLATED FROM ANIMATION TREE */}
                                            <div className="space-y-3">
                                                <label className="text-xs font-black uppercase text-slate-500">Internal Message</label>
                                            ...
                                            <div className="space-y-3">
                                                <label className="text-xs font-black uppercase text-slate-500">External Message</label>
                                                <div className="border border-slate-800 rounded-2xl overflow-hidden bg-slate-950">
                                                    <RichTextToolbar onCommand={applyCommand} />
                                                    <div 
